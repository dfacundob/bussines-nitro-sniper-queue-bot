...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (check == false) {
            ...
            var logEmbed = await client.utils.newEmbed("Add balance")
                .addFields({ name: "User", value: `<@${interaction.user.id}> ||(${interaction.user.id})||`, inline: false })
                .addFields({ name: "User added bal", value: `<@${user.id}> ||(${user.id})||`, inline: false })
                .addFields({ name: "Amount", value: `${amount}`, inline: false })
                .addFields({ name: "Balance", value: `${kaka[guildlog].credits || 0}`, inline: false })
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            client.utils.sendLog(client.server.id, client, logEmbed, kaka[client.server.id].data['Add balance'], 'Add balance')
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

            await interaction.editReply({ content: '`✔️` Balance added.', ephemeral: true });
        }
